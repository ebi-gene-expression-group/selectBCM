...
#' @export
Rank_RNAseq.plot <-function(evaluation)

{
 ...
 # evaluation <-  purrr::keep(evaluation, ~all(!is.na(.x)))
...
parallelCoordinatePlot <- if (c("gender") %in% names(data.frame(w3_m))=="TRUE")
  ...
  {ggparcoord( data=w3_m, columns=1:8,groupColumn = "method",scale = "globalminmax",
               showPoints=TRUE, alphaLines=0.4, splineFactor=10 ) + scale_y_reverse() +scale_color_brewer(palette = "Dark2")+
...
{ggparcoord( data=w3_m, columns=1:7,groupColumn = "method",scale = "globalminmax",
             showPoints=TRUE, alphaLines=0.4, splineFactor=10 ) + scale_y_reverse() +scale_color_brewer(palette = "Dark2")+
    ggtitle( paste( "Diagnostic plot", sep="" ) ) +
    xlab("Evaluation method") +
    ylab( "Rank" ) +
